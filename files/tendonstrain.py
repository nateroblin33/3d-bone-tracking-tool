...
# Boolean to determine if a frame has been viewed yet
start = 0

# Boolean to determine if a frame is bad (missing at least one dot)
badframe = 0

#BOOLEAN FOR STRAIN PLOTTING
strainplot = 0

#COUNTER FOR STRAIN TIME
straintimecount = 0.0
...
while True:
    ...
    if key == ord("q"):
        ...
        #FOR STRAIN
        if strainplot == 1:
            fig = plt.figure('Measured Data')
            fig.set_size_inches(20,20)
            
            figsub1 = fig.add_subplot(1,3,1)
            figsub1.clear()
            figsub1.plot(straintime, strain1, 'g', label='Green to Blue')
            figsub1.plot(straintime, strain2, 'y', label='Yellow to Blue')
            figsub1.set_xlabel('Time (s)')
            figsub1.set_ylabel('Strain (%)')
            figsub1.set_title('Strain vs Time')
            figsub1.grid()
            figsub1.legend()

            figsub2 = fig.add_subplot(1,3,2)
            figsub2.clear()
            figsub2.plot(straintime, strain1len, 'g', label='Green to Blue')
            figsub2.plot(straintime, strain2len, 'y', label='Yellow to Blue')
            figsub2.set_xlabel('Time (s)')
            figsub2.set_ylabel('Length (mm)')
            figsub2.set_title('Length vs Time')
            figsub2.grid()
            figsub2.legend()

            figsub3 = fig.add_subplot(1,3,3)
            figsub3.clear()
            figsub3.plot(straintime, strain1lenplus, 'g', label='Green to Blue')
            figsub3.plot(straintime, strain2lenplus, 'y', label='Yellow to Blue')
            figsub3.set_xlabel('Time (s)')
            figsub3.set_ylabel('Change from Starting Length (mm)')
            figsub3.set_title('Change from Starting Length vs Time')
            figsub3.grid()
            figsub3.legend()
            ...
            fig.savefig('tendonStrain' + dtFix + '.png', dpi=100)

            f = open('tendonStrainData' + dtFix + '.txt', 'w')
            f.write("[Time]" + "\t" + "[Strain 1 (%)]" + "\t" + "[Length 1 (mm)]" + "\t" + "[Change in Length 1 (mm)]" + "\t" + "[Strain 2 (%)]" + "\t" + "[Length 2 (mm)]" + "\t" + "[Change in Length 2 (mm)]" + "\t" + "\n")
            i = 0
            while i < len(straintime):
                #print(i)
                f.write(str(straintime[i]) + "\t" + str(strain1[i]) + "\t" + str(strain1len[i]) + "\t" + str(strain1lenplus[i]) + "\t" + str(strain2[i]) + "\t" + str(strain2len[i]) + "\t" + str(strain2lenplus[i]) + "\t" + "\n")
                i = i + 1
            f.close()
    ...
    if strainplot == 1:
        ...
        #CALCULATE DISTANCES CURRENT VALUES FOR STRAIN
        distGB = euclideanDist(pointg1, pointb1)*3/2
        distYB = euclideanDist(pointy1, pointb1)*3/2
        
        #CALCULATE STRAIN FOR STRAIN
        strain1.extend([(distGB/baseDistGB)*100])
        strain2.extend([(distYB/baseDistYB)*100])
        strain1len.extend([distGB*1000])
        strain2len.extend([distYB*1000])
        strain1lenplus.extend([(distGB-baseDistGB)*1000])
        strain2lenplus.extend([(distYB-baseDistYB)*1000])
        ...
        time2 = time3[0].split(':')
        dtFix = day[0] + "_" + time2[0] + "_" + time2[1] + "_" + time2[2] + "_" + time3[1]
        dtCalc = dtFix.split('_')
        dtCalc2 = dtCalc[3] + "." + dtCalc[4]
        dtCalc3 = float(dtCalc2)
        dtCalc3 = time.time()
        
        dtCalcDiff = dtCalc3 - dtCalc3Base
        print("dtCalcDiff:")
        print(dtCalcDiff)
        
        #UPDATE TIME FOR STRAIN
        straintimecount = dtCalcDiff
        straintime.extend([straintimecount])
        ...
        figsub1 = fig.add_subplot(1,3,1)
        figsub1.clear()
        figsub1.plot(straintime, strain1, 'g', straintime, strain2, 'y')
        figsub1.set_xlabel('Time (s)')
        figsub1.set_ylabel('Strain (%)')
        figsub1.set_title('Strain vs Time')
        ...
        figsub2 = fig.add_subplot(1,3,2)
        figsub2.clear()
        figsub2.plot(straintime, strain1len, 'g', straintime, strain2len, 'y')
        figsub2.set_xlabel('Time (s)')
        figsub2.set_ylabel('Length (mm)')
        figsub2.set_title('Length vs Time')
        ...
        figsub3 = fig.add_subplot(1,3,3)
        figsub3.clear()
        figsub3.plot(straintime, strain1lenplus, 'g', straintime, strain2lenplus, 'y')
        figsub3.set_xlabel('Time (s)')
        figsub3.set_ylabel('Change from Starting Length (mm)')
        figsub3.set_title('Change from Starting Length vs Time')
    ...
    if key == ord("c"):
        ...
        if badframe == 0:
            ...
            #CALCULATE DISTANCES ORIGINAL VALUES FOR STRAIN
            baseDistGB = euclideanDist(pointg1, pointb1)*3/2
            baseDistYB = euclideanDist(pointy1, pointb1)*3/2
            strainplot = 1
            
            #CALCULATE STRAIN FOR STRAIN
            strain1.extend([(baseDistGB/baseDistGB)*100])
            strain2.extend([(baseDistYB/baseDistYB)*100])
            strain1len.extend([baseDistGB*1000])
            strain2len.extend([baseDistYB*1000])
            strain1lenplus.extend([(baseDistGB-baseDistGB)*1000])
            strain2lenplus.extend([(baseDistYB-baseDistYB)*1000])
            ...
            dtCalc = dtFix.split('_')
            dtCalc2 = dtCalc[3] + "." + dtCalc[4]
            dtCalc3Base = float(dtCalc2)
            dtCalc3Base = time.time()
    ...
    # If any color is being tracked, (for one frame every 2 seconds) display all Depth/Coord values, Euclidean Distance values between Black, Red, and Purple, and Roll/Yaw/Pitch values every 1/5 of a second; then, reset all coordinates to (0,0)
        ...
        if ptrue == 1:
            ptrue += 1
            pfalse = 1
        elif ptrue == 40:#Prints all information for one frame every two seconds (while any color is in frame)
            ptrue = 1
            pfalse = 1
        else:
            ptrue += 1
